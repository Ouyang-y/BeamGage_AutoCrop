%   该版本修复了多图分辨率不同导致截图不同，但后来细想不同分辨率本就是不对的。
% 质心截图
%   ①去边(截图)
%   ②将原彩图转为灰度图
%   ③取灰度图质心
...
% select crop range
% save resolution, if not equal, reselect
[fig,rect,resolution] = select_crop_range([path,img_path{1}]);
...
for tt = 1:length(img_path_s)
    ...
    [resolution_temp(1),resolution_temp(2),~] = size(img);
    if any(resolution_temp-resolution),[fig,rect,resolution] = select_crop_range([path,img_path{tt}]);end
    img = imcrop(img,rect);
...
function [fig,rect,resolution] = select_crop_range(path)
% select crop range
img = imread(path);img = img(:,:,1:3);
[resolution(1),resolution(2),~] = size(img);
fig = figure;
